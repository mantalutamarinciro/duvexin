...
export function QuotePDF({ data, quote }: QuotePDFProps) {
  ...
  return <div className="bg-white">
    <section style={{ ...pageStyle, padding: "4mm", border: `2px solid ${teal}` }}>
      <div className="grid grid-cols-[52mm_1fr_62mm] items-center gap-2 pt-6">
        <div className="rounded-[18px] border p-4 italic leading-[1.45]" style={{ borderColor: teal, color: teal, fontFamily: "Georgia, serif", fontSize: "12px" }}><div className="font-bold">DEVIS N° {number}</div><div>Date : {safeDate(new Date())}</div><div>Code client : {data.clientCode || "À CRÉER"}</div><div>Devis valable 1 mois</div></div>
        <div className="text-center"><img src="/images/logo.png" alt="Déménagement du Vexin" style={{ width: "88mm", height: "auto", margin: "0 auto" }} /><div className="mt-1 flex justify-center gap-8 text-left text-[9px] font-bold" style={{ color: teal }}><span>9 rue de Pontoise<br />95540 MERY-SUR-OISE</span><span>TEL : 01 30 75 12 35<br />MOB : 07 68 31 33 10</span></div></div>
        <div className="rounded-[18px] border p-4 text-center italic leading-[1.45]" style={{ borderColor: teal, color: teal, fontFamily: "Georgia, serif", fontSize: "12px" }}><div className="underline">Adresse du Client</div><div>{data.clientName}</div><div>{clientAddress}</div></div>
      </div>
      <div className="px-9 pt-16 text-[11px] leading-[1.55]">
    ...
    <section style={{ ...pageStyle, padding: "7mm 8mm 4mm", border: `2px solid ${teal}`, pageBreakAfter: "auto" }}>
